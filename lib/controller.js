		...
		success: function(response) {
			$.each(response, function(i, task){
				...
				for (var i = 0; i < task.current_block; i++)
					$taskLi.find(".task-blocks-list").append("<li><button class='task-done-block'></button></li>");
				if (task.current_block < task.blocks)
					$taskLi.find(".task-blocks-list").append("<li><button class='task-current-block'></button></li>");
				for (var i = task.current_block + 1; i < task.blocks; i++)
					$taskLi.find(".task-blocks-list").append("<li><button class='task-undone-block'></button></li>");
